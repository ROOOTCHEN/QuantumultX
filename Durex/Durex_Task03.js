...
class UserInfo {
	...
	async signin() {
		try {
			const options = {
				url: `https://vip.ixiliu.cn/mp/activity.lottery/draw?snId=374670664476672&channelSn=0`,
				headers: {
			...
			if (result?.code === -1) {
				$.log(`❌重复抽奖！`);
				$.signMsg = `❌重复抽奖`;
			} else if (result?.status === 200 && result?.data?.prize) {
				const prizeName = result.data.prize.prize_name;
				$.log(`✅抽奖成功! 获得的奖品是: ${prizeName}`);
				$.signMsg = `✅抽奖成功! 获得的奖品是: ${prizeName}`;
			} else {
				// 如果不是重复签到，也不是成功签到的情况，可以输出一个默认消息  
				$.log(`奖品结果未知`);
				$.signMsg = `奖品结果未知`;
			}
		} catch (e) {
...
async function getCookie() {
		if ($request && $request.method != 'OPTIONS') {
			const tokenValue = $request.headers['Access-Token'] || $request.headers['authorization'] || $request
